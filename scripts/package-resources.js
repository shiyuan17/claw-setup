...
function parseArgs() {
  ...
  for (let i = 0; i < args.length; i++) {
    if (args[i] === "--platform" && args[i + 1]) {
      ...
      die("claw-setup 独立资源打包不支持 gateway.asar；请使用默认散文件 resources/targets");
    }
  }
  ...
  if (process.env.ONECLAW_GATEWAY_ASAR === "1") {
    die("claw-setup 独立资源打包不支持 ONECLAW_GATEWAY_ASAR=1");
  }
  return opts;
}
...
function extractDarwin(tarPath, runtimeDir, version, arch, targetId) {
  ...
  // 拷贝 lib/node_modules/npm/ 到 vendor/npm/（避免后续打包器过滤 node_modules）
...
function installTgzPluginDeps(plugin, pluginDir, targetId, opts) {
  ...
  // 必须 target 随包 Node.js runtime 的 Node ABI（gateway 由 bundled Node 启动）
  ...
  if (nativeAddonPkgs.length > 0) {
    ...
    log(`为 ${plugin.id} 编译 native addon: ${nativeAddonPkgs.join(", ")} (arch=${opts.arch}, node=${nodeVersion})`);
    for (const pkg of nativeAddonPkgs) {
      try {
        execSync(`npm rebuild ${pkg} --arch=${opts.arch} --runtime=node --target=${nodeVersion} --dist-url=https://nodejs.org/download/release`, {
          cwd: depTmpDir,
          stdio: "inherit",
        });
      } catch (err) {
...
// ─── Step 6: Gateway 归档打包（claw-setup 默认禁用） ───
...
function verifyAsarContents(asarPath) {
  void asarPath;
  die("claw-setup 独立资源打包不支持 gateway.asar");
}
...
async function main() {
  ...
  // Step 6: Gateway 资源保留为散文件，供 Tauri 直接打包
  if (opts.asar) {
    log("Step 6: Gateway 归档打包");
    await packGatewayAsar(targetPaths.gatewayDir, targetPaths.targetBase, opts.platform, opts.arch);
  } else {
    log("Step 6: 使用散文件 resources/targets");
  }
